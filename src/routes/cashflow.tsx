  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <div className="grid grid-cols-2 gap-2 md:grid-cols-4">
        ...
        <Kpi label="NET" value={formatUsd(stats.totals.net)} tone={stats.totals.net >= 0 ? "gain" : "loss"} />
      ...
      <Panel title="RECURRING · MAPPED TO ASSETS">
        {data.assets.length === 0 ? (
          ...
          <>
                ...
                {recPager.slice.map((r) => (
                  <tr key={r.id} className="border-b border-border/50">
                    ...
                    <td className="truncate py-1.5 pr-2">
                      <Link to="/assets/$id" params={{ id: r.assetId }} className="text-accent hover:underline">
            ...
            <Pager page={recPager.page} totalPages={recPager.totalPages} total={recPager.total} from={recPager.from} to={recPager.to} onChange={recPager.setPage} />
          </>
        )}
      </Panel>
            ...
            {stats.series.some((m) => m.in > 0 || m.out > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={stats.series} margin={{ top: 4, right: 4, left: 0, bottom: 0 }}>
                  ...
                  <YAxis hide />
                  <Tooltip contentStyle={{ background: "#000", border: "1px solid #ff6d00", borderRadius: 0, fontSize: 11, fontFamily: "IBM Plex Mono" }} formatter={(v: number, name: string) => [formatUsd(v), name === "in" ? "IN" : "OUT"]} />
      ...
      <div className="grid gap-2 lg:grid-cols-2">
        <Panel title="SCHEDULE · NEXT EVENTS">
            ...
            <tbody>
              {eventPager.slice.map((e, i) => (
                <tr key={`${e.date}-${e.name}-${i}`} className="border-b border-border/50">
                  ...
                  <td className="truncate py-1 text-subtle">{assetById.get(e.assetId) || "—"}</td>
          ...
          <Pager page={eventPager.page} totalPages={eventPager.totalPages} total={eventPager.total} from={eventPager.from} to={eventPager.to} onChange={eventPager.setPage} />
        </Panel>
            ...
            <tbody>
              {stats.cats.slice(0, 10).map((c) => (
                <tr key={c.name} className="border-b border-border/50">
                  ...
                  <td className={`py-1 text-right tabular-nums ${c.value >= 0 ? "text-gain" : "text-loss"}`}>
                    {formatUsd(c.value)}
                  </td>
                </tr>
              ))}
      ...
      <Panel title="LEDGER">
            ...
            <li key={t.id} className="flex items-center justify-between gap-3 px-1 py-1.5">
              <div className="min-w-0">
                ...
                <p className="text-[10px] text-subtle">
                  ...
                  {t.assetId && assetById.get(t.assetId) ? ` · ${assetById.get(t.assetId)}` : ""}
                </p>
              </div>
        ...
        <Pager page={txPager.page} totalPages={txPager.totalPages} total={txPager.total} from={txPager.from} to={txPager.to} onChange={txPager.setPage} />
      </Panel>
      ...
      <Dialog open={recOpen} onOpenChange={setRecOpen} title="Mapear flujo recurrente">
        ...
        >
          <Field label="Activo (obligatorio)">
            <Select required value={recAssetId} onChange={(e) => setRecAssetId(e.target.value)}>
              <option value="" disabled>Elegí un activo cargado…</option>
              {data.assets.map((a) => (
          ...
          <Field label="Concepto">
            <Input required value={recName} onChange={(e) => setRecName(e.target.value)} placeholder="Alquiler, cupón, dividendo…" />
          </Field>
      ...
      <Dialog open={open} onOpenChange={setOpen} title="Nuevo movimiento">
        <form
          ...
          onSubmit={async (e) => {
            ...
            const signed = type === "EXPENSE" || type === "BUY" || type === "TRANSFER" ? -Math.abs(raw) : Math.abs(raw);
            await addTransaction({
              data: { description: desc, amount: signed, currency, type, category: category || type, date },
            });
...
function Kpi({ label, value, tone }: { label: string; value: string; tone: "gain" | "loss" | "accent" }) {
  const color = tone === "gain" ? "text-gain" : tone === "loss" ? "text-loss" : "text-accent";
  return (
